...
const DEFAULT_MULTI_AGENT_V2_WAIT_AGENT_USAGE_HINT_TEXT: &str = "When your own work is finished and you are waiting for agents, call `wait_agent` without `timeout_ms`. In codex-v2, omitting the timeout waits for actionable input, agent completion or failure, or user steering without periodic model calls. Use an explicit `timeout_ms` only when a bounded wait is required. Do not routinely poll with `list_agents`; use it for deliberate inspection or diagnosis.";
